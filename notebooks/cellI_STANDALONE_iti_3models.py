# =============================================================================
# EXTRA CELL I — STANDALONE: canonical-scorer ITI on Mistral + OLMo + Llama
# (GPU, ~2h on A100 — pick Runtime > Change runtime type > A100 first!)
# NO other cells needed. Paste whole cell into any fresh GPU kernel, run, walk away.
# Completes the ITI frontier across all four families with ONE scorer.
# Each model's results save to Drive as it finishes — a crash loses at most one model.
# =============================================================================
import os, json, re
import numpy as np
try:
...
for MNAME, JPATH, SLUG in MODELS:
  ...
  json.dump({'model': MNAME, 'scorer': 'canonical_correction_aware', 'baseline_POS': b_pos,
             'baseline_FP': b_fp, 'results': results},
            open(f'{OUT}/iti_canonical_{SLUG}.json', 'w'), indent=2)
  json.dump(all_gens, open(f'{OUT}/iti_canonical_generations_{SLUG}.json', 'w'))
  print(f"Saved -> iti_canonical_{SLUG}.json")
