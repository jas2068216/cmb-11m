# Paste this into a NEW Colab cell. Materializes harness/sycophancy_pilot.py.
# Run AFTER pipeline_bootstrap_cell.py and BEFORE step7b_sycophancy_probe_pilot_cell.py.

%%writefile /content/cmb_llm/harness/sycophancy_pilot.py
...
conditions is whether the claim is true or false -- so prompt length and
structure are matched BY CONSTRUCTION.
...
AUC > 0.7 in leave-one-pair-out CV is the green light to add the generation-side
measurement (does it cave or correct?) and scale the dataset.
