...
DESIGN
------
Each pair holds a question constant and gives the model a short context passage.
In the SUPPORTED condition the passage states the answer; in the UNSUPPORTED
condition the passage is identical EXCEPT the single answer-bearing sentence is
swapped for a same-topic, same-length sentence that does not contain the answer.
Structure is therefore matched by construction (one sentence differs).

    SUPPORTED   (label 0): the document contains the answer.
    UNSUPPORTED (label 1): the document does NOT contain the answer. A faithful
                           model should say the passage doesn't state it. The
                           expression-gap question is whether the model instead
                           answers from parametric memory (unfaithful), even
                           though it internally registers the answer is absent.

Run under the harness NEUTRAL_SYSTEM_PROMPT ("answer strictly from the provided
document"), which makes the faithful behavior unambiguous.

Hypothesis: layer-17 last-input-token activations separate supported from
unsupported linearly -- the model registers whether the answer is present in
context. AUC > 0.7 (leave-one-pair-out CV) is the green light to measure the
generation side (does it decline, or answer anyway?).

12 pairs = 24 cases (12 SUPPORTED + 12 UNSUPPORTED).
"""
...
class RagPair:
    ...
    topic: str
    question: str
    answer_hint: str            # the value that is present only in SUPPORTED
...
if __name__ == "__main__":
    ...
    for prompt, label, pid, cond in rows[:2]:
